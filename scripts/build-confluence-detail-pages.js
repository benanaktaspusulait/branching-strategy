...
function removeRepositoryOnlyReferences(markdown) {
  return markdown
    ...
    .replace(/\bdocument set that could support\b/gi, 'Confluence page set that could support')
    ...
    .replace(/: source (\[[^\]]+\]\([^)]+\))/g, ': $1')
    .replace(/\bas the detailed source for\b/g, 'as the detailed page for')
    .replace(/\bSource architecture detail\b/g, 'Architecture detail')
